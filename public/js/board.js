...
export async function loadVisibleBoardDates(forceReload, options = {}) {
  ...
  try {
    ...
    dates.forEach((date) => {
      MEMBERS.forEach((member) => {
        const key = cellKey(date, member);
        if (preserveDraft && Object.prototype.hasOwnProperty.call(state.boardDraft, key)) {
...
export async function saveBoardChanges() {
  ...
  try {
    ...
    await loadVisibleBoardDates(true, { preserveDraft: false });
